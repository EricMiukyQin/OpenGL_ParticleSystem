...
FireworkCylinder::FireworkCylinder(ShaderManager* shaderManager, const glm::vec3& trans, const glm::vec3& scale, const int particleNumber, const float particleLifespan, const glm::vec4& color)
...
FireworkCylinder::~FireworkCylinder() {}


void FireworkCylinder::Update(const float delta_t) {
	for (int i = 0; i < mParticlePool.size(); ++i) {
		...
		if (Util::Equal(mParticlePool[i]->velocity.x, 0.f) && Util::Equal(mParticlePool[i]->velocity.z, 0.f)) {
			if (Util::GreaterOrEqual(mParticlePool[i]->position.y, 300.f) && Util::RandFloat(-4.f, 1.f) > 0) {
				...
				// Cylindrical rise.
				mParticlePool[i]->position.x = 10.f * cosf(i * 2 * PI / mParticlePool.size());
				mParticlePool[i]->position.y += mParticlePool[i]->velocity.y * delta_t + .5f * mParticlePool[i]->gravity * powf(delta_t, 2);
				mParticlePool[i]->position.z = 10.f * sinf(i * 2 * PI / mParticlePool.size());
			}
		}
		else {
			// phase 2
...
void FireworkCylinder::BuildArrays() {
	for (const auto& particle : mParticlePool)
		particle->sphere->BuildArrays();
}

void FireworkCylinder::BindVBO(unsigned int programID) {
	for (const auto& particle : mParticlePool)
		particle->sphere->BindVBO(programID);
}


void FireworkCylinder::RenderParticle(FireworkParticle* particle) {
