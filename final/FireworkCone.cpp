...
FireworkCone::FireworkCone(ShaderManager* shaderManager, const glm::vec3& trans, const glm::vec3& scale, const int particleNumber, const float particleLifespan, const glm::vec4& color)
...
FireworkCone::~FireworkCone() {}


void FireworkCone::Update(const float delta_t) {
	for (int i = 0; i < mParticlePool.size(); ++i) {
		...
		if (Util::Equal(mParticlePool[i]->velocity.x, 0.f) && Util::Equal(mParticlePool[i]->velocity.z, 0.f)) {
			if (Util::LessOrEqual(mParticlePool[i]->velocity.y, 80.f)) {
				// assign a phase 2 velocity
				mParticlePool[i]->velocity = glm::vec3{ 30.f * cosf(2.f * i * PI / mParticlePool.size()) + Util::RandFloat(-2.5f, 2.5f), 15.f + Util::RandFloat(-5.f, 5.f), 30.f * sinf(2.f * i * PI / mParticlePool.size()) + Util::RandFloat(-2.5f, 2.5f) };
			}
			else {
				// in phase 1
...
void FireworkCone::BuildArrays() {
	for (const auto& particle : mParticlePool)
		particle->sphere->BuildArrays();
}

void FireworkCone::BindVBO(unsigned int programID) {
	for (const auto& particle : mParticlePool)
		particle->sphere->BindVBO(programID);
}

void FireworkCone::RenderParticle(FireworkParticle* particle) {
