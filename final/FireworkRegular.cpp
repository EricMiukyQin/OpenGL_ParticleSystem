...
FireworkRegular::FireworkRegular(ShaderManager* shaderManager, const glm::vec3& trans, const glm::vec3& scale, const int particleNumber, const float particleLifespan, const glm::vec4& color)
...
FireworkRegular::~FireworkRegular() {}


void FireworkRegular::Update(const float delta_t) {
	for (int i = 0; i < mParticlePool.size(); ++i) {
		...
		if (Util::Equal(mParticlePool[i]->velocity.x, 0.f) && Util::Equal(mParticlePool[i]->velocity.z, 0.f)) {
			...
			if (Util::GreaterOrEqual(mParticlePool[i]->position.y, 300.f) && Util::RandFloat(-4.f, 1.f) > 0) {
				...
				// update position. Original
				mParticlePool[i]->position.x += Util::RandFloat(-.2f, .2f);
				mParticlePool[i]->position.y += mParticlePool[i]->velocity.y * delta_t + .5f * mParticlePool[i]->gravity * powf(delta_t, 2);
				mParticlePool[i]->position.z += Util::RandFloat(-.2f, .2f);
			}
		} else {
			// phase 2
...
void FireworkRegular::BuildArrays() {
	for (const auto& particle : mParticlePool)
		particle->sphere->BuildArrays();
}

void FireworkRegular::BindVBO(unsigned int programID) {
	for (const auto& particle : mParticlePool)
		particle->sphere->BindVBO(programID);
}


void FireworkRegular::RenderParticle(FireworkParticle* particle) {
